...
class ServiceNowConnector {
   ...
   * @memberof ServiceNowConnector
   * @method constructUri
   * @summary Build and return the proper URI
 ...
 processRequestResults(error, response, body, callback) {
    ...
    */
   if(!response.statusCode.toString().match(validResponseRegex)) {
      error = `Invalid response code returned:  ${response.statusCode}`;
     }
     if(this.isHibernating(response)) {
      error = `Instance is hibernating`;
     }
  ...
  * @memberof ServiceNowConnector
  * @method sendRequest
  * @summary Builds final options argument
